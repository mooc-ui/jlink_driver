...
const char *trick = __HOUXDs_TRICK__;
const int trick_size = sizeof(__HOUXDs_TRICK__)-1;
...
int main(int argc, char *argv[])
{
	...
	char cmd[512];
	sprintf(cmd, "copy \"%s\" \"./prog_%s_%s_%s.bin\"", argv[1], version_str, rel_type, build_time);
	res = system(cmd);
	if (res) {
	...
	sprintf(tmp,
		...
		"build type: %s\r\n"
		"build time: %s\r\n"
		, version_str, rel_type, build_time
	);
