...
int main(int argc, char *argv[])
{
	...
	int rd = fread(buf, 1, flen, f);
	if (flen != rd) {
		free(buf);
		printf("error: file read error want %u read %u.\r\n",flen,rd);
		return -4;
	}
	uint16_t crc = calc_crc(buf, flen);
	fseek(f, 0L, SEEK_END);
	...
	char fout[512];
	sprintf(fout, "%s%s", fin, "_crc16_info.txt");
	FILE *fo = fopen(fout, "w+");
	if (fo == NULL) {
		printf("warning: cannot create info file.\r\n");
	}
	else {
		fseek(fo, 0L, SEEK_SET);
		fprintf(fo, "File Size: %u Byte\r\nCrc16 Value: 0x%04X\r\n", flen, crc);
		fflush(fo);
		fclose(fo);
	}

	printf("File Size:%u Byte,  Crc16 Val:%04X\r\n", flen, crc);
    return 0;
}
